...
import { Camera, Star, Heart, ArrowRight, Sparkles } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-3xl mx-auto w-full gap-8">
        ...
        >
          <div className="inline-block animate-float mb-4">
            <span className="bg-primary/20 text-primary px-4 py-1.5 rounded-full text-sm font-bold font-display uppercase tracking-wider flex items-center gap-2">
              <Star size={14} fill="currentColor" /> Web Photobooth <Star size={14} fill="currentColor" />
            </span>
          </div>
          <h1 className="text-5xl sm:text-7xl font-display font-extrabold text-foreground leading-tight drop-shadow-sm">
            Capture Your <br/>
          ...
          <p className="text-lg text-muted-foreground font-medium max-w-lg mx-auto">
            A playful digital photobooth with Y2K vibes, cute filters, and animated glitter.
          </p>
        </motion.div>
        ...
        <motion.div 
          ...
          className="w-full"
        >
          <Card className="bg-gradient-to-br from-white to-pink-50 border-primary/20 overflow-hidden relative">
            ...
            <CardContent className="p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6">
              <div className="flex-1 space-y-2 text-center sm:text-left">
                <div className="flex items-center justify-center sm:justify-start gap-2 text-primary font-bold">
                  ...
                  <span>Dani's Daily Theme</span>
                </div>
                {isLoading ? (
                  <div className="h-16 flex items-center justify-center sm:justify-start">
                    <span className="animate-pulse text-muted-foreground">Loading today's vibe...</span>
                  </div>
                ) : dailyTheme ? (
                  <>
                    <h3 className="text-2xl font-display font-bold">{dailyTheme.emoji} {dailyTheme.title}</h3>
